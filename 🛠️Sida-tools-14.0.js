...
// @version      14.0
// @description  داشبورد کشویی ابزارهای کمکی سیدا
// @author       You
...
<\/script></body></html>`;
        ...
        function createPanel() {
            ...
            panel.style.cssText = 'position:fixed;top:20px;left:20px;background:#fff;border:2px solid #3b82f6;border-radius:10px;padding:15px;width:320px;z-index:999999;box-shadow:0 4px 20px rgba(0,0,0,0.2);font-family:Tahoma,sans-serif;direction:rtl;font-size:13px;color:#333;';
    ...
    function extractClassListTool() {
        ...
        function extractFromTable(table) {
            ...
            console.log(`📋 ایندکس‌ها: کد=${idx.code}, نام=${idx.name}, نام خانوادگی=${idx.family}`);
            ...
            rows.forEach(function(row, rowIndex) {
                ...
                if (rowIndex === 0) {
                    console.log(`🔍 ردیف اول: کد="${code}", نام="${name}", نام خانوادگی="${family}"`);
                }
            ...
            console.log(`✅ ${students.length} دانش‌آموز استخراج شد`);
            if (students.length > 0) {
                console.log('📋 نمونه:', students[0]);
            }
            return students;
        }
        ...
        function createPanel() {
            ...
            panel.style.cssText = 'position:fixed;top:20px;left:20px;background:#1a1d2e;border:2px solid #3ecfe0;border-radius:12px;padding:16px 18px;width:380px;z-index:999999;box-shadow:0 4px 20px rgba(0,0,0,0.4);font-family:Tahoma,sans-serif;direction:rtl;font-size:13px;color:#e8e8f0;';
            ...
            document.getElementById('btnCollect').onclick = function() {
                ...
                console.log('📋 نمونه دانش‌آموز:', students[0]);
  ...
  // ==================== ابزار ۱۴: بررسی ملیت والدین ====================
    function nationalityCheckTool() {
        ...
        function getRowNumber(row) {
            ...
            // بررسی: آیا اصلاً ردیفی در جدول هست؟
            let totalRows = 0;
            for (let row of rows) {
                let cols = row.querySelectorAll('td');
                if (cols.length > Math.max(col.code, col.name, col.family)) {
                    totalRows++;
                }
            }
            
            // اگه هیچ ردیفی نبود، null برگردون
            if (totalRows === 0) {
                return null;
            }
            ...
            // ریست startRowNum برای صفحه بعد
            status.startRow = 1;
            saveStatus();
        ...
        async function checkNationality() {
            ...
            if (!student) {
                ...
                if (hasRows) {
                    ...
                    autoMode = false;
                    isPaused = false;
                    let total = completedStudentsList.length + incompleteStudentsList.length;
                    updatePanelUI('', '✅ پایان کار');
                    showNotification('🎉 بررسی کامل شد! مجموع: ' + total + ' دانش‌آموز', 0);
                    alert('✅ بررسی کامل شد!\n\n✅ تکمیل شده: ' + completedStudentsList.length + '\n❌ تکمیل نشده: ' + incompleteStudentsList.length + '\n📊 مجموع: ' + total);
                }
                return;
            }
            ...
            let spanElements = document.querySelectorAll('span.k-icon.k-i-arrow-e');
            for (let btn of spanElements) {
                if (!btn.closest('.k-state-disabled') && !btn.closest('.disabled')) {
                    nextBtn = btn;
                    break;
                }
            }
            ...
            if (!nextBtn) {
                let allAnchors = document.querySelectorAll('a');
                for (let a of allAnchors) {
                    if (a.querySelector('span.k-i-arrow-e') && !a.classList.contains('k-state-disabled') && !a.classList.contains('disabled')) {
                        nextBtn = a;
                        break;
                    }
                }
            ...
            if (!nextBtn) {
                ...
                for (let el of allElements) {
                    ...
                    if ((text === 'بعدی' || ariaLabel.includes('بعدی') || title.includes('بعدی')) && 
                        !el.closest('.k-state-disabled') && !el.classList.contains('disabled')) {
                        nextBtn = el;
                        break;
                    }
                }
        ...
        function showFinalReport() {
            ...
            if (completedStudentsList.length > 0) {
                html += '<div style="overflow-x:auto;margin-bottom:25px;"><table style="width:100%;border-collapse:collapse;font-size:13px;text-align:center;"><thead><tr style="background:#2e7d32;color:white;"><th style="padding:8px;border:1px solid #1b5e20;">ردیف</th><th style="padding:8px;border:1px solid #1b5e20;">نام خانوادگی</th><th style="padding:8px;border:1px solid #1b5e20;">نام</th><th style="padding:8px;border:1px solid #1b5e20;">کد ملی</th></tr></thead><tbody>';
            ...
            if (incompleteStudentsList.length > 0) {
                html += '<div style="overflow-x:auto;margin-bottom:25px;"><table style="width:100%;border-collapse:collapse;font-size:13px;text-align:center;"><thead><tr style="background:#ef4444;color:white;"><th style="padding:8px;border:1px solid #c62828;">ردیف</th><th style="padding:8px;border:1px solid #c62828;">نام خانوادگی</th><th style="padding:8px;border:1px solid #c62828;">نام</th><th style="padding:8px;border:1px solid #c62828;">کد ملی</th></tr></thead><tbody>';
