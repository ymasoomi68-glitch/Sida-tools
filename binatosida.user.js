...
(function(){
    ...
    console.log('🚀 اسکریپت جمع‌آوری خودکار از بینا v3.2');
    ...
    function exportLogToWord(collectLogs){
        ...
        if(collectLogs && collectLogs.length){
            // تبدیل آرایه آبجکت به متن ساده برای Word
    ...
    function createFloatingButton(){
        ...
        btn.onmouseenter = function(){
            this.style.boxShadow = '0 6px 20px rgba(102, 126, 234, 0.7)';
        };
    ...
    function createPanel(){
        ...
        header.innerHTML = `
            ...
            <div style="display:flex;gap:6px;">
                <span id="cp-min" title="مینیمایز" style="cursor:pointer;font-size:17px;padding:4px 10px;background:rgba(255,255,255,0.15);border-radius:6px;">➖</span>
                <span id="cp-reset" title="ریست پنل" style="cursor:pointer;font-size:17px;padding:4px 10px;background:rgba(255,255,255,0.15);border-radius:6px;">🔄</span>
                <span id="cp-close" title="بستن" style="cursor:pointer;font-size:17px;padding:4px 10px;background:rgba(255,255,255,0.15);border-radius:6px;">✖</span>
            </div>
        `;
        ...
        status.style.cssText = `
            ...
            font-size: 15px; color: #94a3b8; border-bottom: 1px solid #334155;
        `;
        ...
        btnCollectSingle.style.cssText = `
            padding: 14px; background: #10b981; color: white; border: none;
        ...
        btnAutoCollect.style.cssText = `
            padding: 16px; background: linear-gradient(135deg,#f59e0b,#dc2626);
        ...
        btnCopyForSida.style.cssText = `
            padding: 14px; background: #0ea5e9; color: white; border: none;
            border-radius: 10px; cursor: pointer; font-size: 16px; font-weight: bold;
            font-family: inherit;
        `;
        ...
        log.style.cssText = `
            background: #0f172a; border-radius: 8px; padding: 12px;
            font-family: Consolas, monospace; font-size: 14px; line-height: 2;
        ...
        var storageType = hasGM() ? 'GM' : 'LocalStorage';
        ...
        function renderStorageList(filter){
            ...
            keys.forEach(function(k){
                ...
                html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px 12px;margin-bottom:6px;background:#1e293b;border-radius:8px;border-right:4px solid #3b82f6;">';
                html += '<div>';
                ...
                html += '</div>';
                html += '<button class="cp-delete-btn" data-key="' + k + '" style="background:#dc2626;color:white;border:none;border-radius:6px;padding:6px 12px;cursor:pointer;font-size:13px;font-family:inherit;">🗑️ حذف</button>';
                html += '</div>';
            });
        ...
        btnCopyForSida.onclick = function(){
            try {
                ...
                function fallbackCopyClipboard(text) {
                    var ta = document.createElement('textarea');
                    ta.value = text;
                    ta.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:80%;height:300px;z-index:99999999;direction:ltr;font-family:monospace;font-size:12px;padding:10px;border:3px solid #0ea5e9;border-radius:10px;';
                    document.body.appendChild(ta);
                    ta.select();
                    ta.focus();
                    try {
        ...
        // ✅ تغییر ۱: پاک کردن لیست همراه انبار
